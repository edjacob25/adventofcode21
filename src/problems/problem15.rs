...
fn a_star_search(
    graph: &HashMap<(isize, isize), Rc<RefCell<Node>>>,
    start: (isize, isize),
    end: (isize, isize),
) -> u32 {
...
fn expand_graph(
    graph: &mut HashMap<(isize, isize), Rc<RefCell<Node>>>,
    x_size: &mut isize,
    y_size: &mut isize,
) {
    ...
    *y_size = max_y + (max_y + 1) * 4;
    for &key in graph.keys() {
        let base = (graph.get(&key).unwrap().borrow() as &RefCell<Node>)
            .borrow()
            .node_value;
        for j in 0..5 {
            for i in 0..5 {
                ...
                let value = Node {
                    node_value,
                    acc_cost: u32::MAX,
                    heuristic: ((*y_size - new_key.1 as isize) + (*x_size - new_key.0 as isize))
                        as u32,
                    best_predecessor: None,
                    pos: new_key,
                };
...
impl Ord for Node {
    fn cmp(&self, other: &Self) -> Ordering {
        if self.acc_cost + self.heuristic < other.acc_cost + other.heuristic {
            Ordering::Greater
        } else if self.acc_cost + self.heuristic > other.acc_cost + other.heuristic {
            Ordering::Less
        } else {
            Ordering::Equal
        }
